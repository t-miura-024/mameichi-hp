...
| 主な表示要素 | ヒーローセクション（キャッチコピー + メインビジュアル）、お店の紹介（概要）、おすすめ・新着の商品（数件ピックアップ）、最新のお知らせ（数件）、Instagram 埋め込み、アクセス情報（簡易）、各ページへの CTA |
...
| URL | `/products` |
| データソース | microCMS |
| 主な表示要素 | 商品の一覧（カード形式）、各商品の詳細情報（名前・産地・焙煎度・特徴・価格帯・画像）、フィルタリング機能（焙煎度、おすすめ、季節限定など）、個別詳細ページ（`/products/[id]`） |
...
**ヘッダー:**
- ロゴ（準備でき次第差し替え）
- ナビゲーションメニュー（トップ / お店について / 商品一覧 / アクセス / お知らせ / お問い合わせ）
- モバイル時：ハンバーガーメニュー
...
| category | カテゴリ | セレクトフィールド | | 新着商品 / イベント / 営業案内 / その他 |
...
1. **日常更新（店主）**: microCMS アプリ（スマホ）→ 記事・商品情報を入力 → 公開ボタン → 自動でサイト更新
